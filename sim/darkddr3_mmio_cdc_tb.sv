...
module darkddr3_mmio_cdc_tb;
    ...
    // DDR-домен: период не кратен CPU-клоку, чтобы CDC работал как на железе
    logic DDRCLK = 1'b0;
    always #3 DDRCLK = !DDRCLK;
    logic DDRRSTN = 1'b0;
    initial begin
        repeat (6) @(posedge DDRCLK);
        DDRRSTN = 1'b1;
    end

    // провода между CDC и моделью (ddr-домен)
    logic c_rd, c_wr, c_refresh;
    logic [DDR_ADDR_WIDTH-1:0] c_addr;
    logic [15:0] c_din;

    logic [15:0] cpu_dout_w;
    logic cpu_ready_w, cpu_busy_w, cpu_wld_w, cpu_rcd_w;
    ...
    darkddr3_mmio #(
        ...
        .ddr_din(ddr_din),
        .ddr_dout(cpu_dout_w),
        .ddr_data_ready(cpu_ready_w),
        .ddr_busy(cpu_busy_w),
        .ddr_write_level_done(cpu_wld_w),
        .ddr_read_calib_done(cpu_rcd_w)
    );
    ...
    darkddr3_ui_cdc #(
        ...
        .cpu_rd(ddr_rd),
        .cpu_wr(ddr_wr),
        .cpu_refresh(ddr_refresh),
        .cpu_addr(ddr_addr),
        .cpu_din(ddr_din),
        .cpu_dout(cpu_dout_w),
        .cpu_data_ready(cpu_ready_w),
        .cpu_busy(cpu_busy_w),
        .cpu_write_level_done(cpu_wld_w),
        .cpu_read_calib_done(cpu_rcd_w),
        .ddr_clk(DDRCLK),
        .ddr_resetn(DDRRSTN),
        .ddr_rd(c_rd),
        .ddr_wr(c_wr),
        .ddr_refresh(c_refresh),
        .ddr_addr(c_addr),
        .ddr_din(c_din),
        .ddr_dout(ddr_dout),
